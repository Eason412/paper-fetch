...
5. **保存状态并支持恢复。** PDF、规范化清单、详细报告和恢复状态都写入输出目录；再次运行同一清单时跳过已经验证的 PDF，只重试未完成项目。
6. **安全分段。** 单次最多尝试 30 篇机构访问；超出的记录写入 `oa_fetch_pending.csv`，必须等用户再次要求后才能继续。
...
| `exists` | 状态记录中的 PDF 已存在且通过 `%PDF` 检查，跳过网络。 |
...
- `*.pdf`：带 canonical identity 哈希后缀的论文；
...
离线测试覆盖配置优先级和权限、manifest 规范化与去重、OA 优先编排、恢复和 pending、原子写入、URL/重定向安全、出版商边界、限速参数及 Skill 契约：
